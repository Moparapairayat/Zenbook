...
const ListingDetail = () => {
  ...
  const shareToFeed = async () => {
    ...
    const content = `Check out this listing: "${listing.title}" — $${Number(listing.price).toLocaleString()}\n\n${listingUrl}`;
  ...
  const handleMessageSeller = async () => {
    ...
    await supabase.from("messages").insert({
      ...
      sender_id: user.id,
      content: `Hi! I'm interested in your listing: "${listing.title}" ($${Number(listing.price).toLocaleString()})`,
    });
            ...
            <div className="rounded-lg bg-card shadow-sm p-5">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem onClick={shareToFeed}>
                        <LinkIcon className="w-4 h-4 mr-2" /> Share to Feed
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                  <button
              ...
              <div className="flex flex-wrap items-center gap-2 mt-3">
                ...
                <Badge variant="outline" className="text-xs">
                  <ShieldCheck className="w-3 h-3 mr-1" /> {listing.condition}
                </Badge>
                {listing.location && (
                  <Badge variant="outline" className="text-xs">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-1">
                    ...
                    >
                      {listing.profile?.display_name || "Unknown"}
                    </Link>
                  ...
                  <div className="flex items-center gap-3 text-xs text-muted-foreground mt-0.5">
                    <span className="flex items-center gap-1">
                      ...
                      Joined {listing.profile?.created_at ? format(new Date(listing.profile.created_at), "MMM yyyy") : "recently"}
                    </span>
                    <span>{listing.sellerListingCount} listing{listing.sellerListingCount !== 1 ? "s" : ""}</span>
                  </div>
                </div>
              </div>
              <Link

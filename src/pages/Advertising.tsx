      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Create, manage, and track your sponsored posts and promotions.
          </p>
        </div>
      ...
      <Tabs defaultValue="dashboard" className="space-y-6">
        ...
        <TabsContent value="dashboard" className="space-y-6">
          ...
          {activePosts.length === 0 && (
            <div className="text-center py-16 rounded-xl border border-dashed border-border">
              ...
              <p className="text-lg font-medium text-foreground">No active ads</p>
              <p className="text-sm text-muted-foreground mt-1 mb-4">
                Create your first sponsored post to reach more people.
              </p>
              <Button onClick={() => setShowCreate(true)} className="gap-2">
        ...
        <TabsContent value="ads" className="space-y-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-center gap-2 flex-wrap">
              <h2 className="text-lg font-semibold text-foreground">My Ads</h2>
              <div className="flex gap-1">
                {["all", "active", "paused", "expired"].map((f) => (
                  ...
                  >
                    {f} {f !== "all" && `(${myAds.filter((a: any) => getAdStatus(a) === f).length})`}
                  </button>
                ))}
              </div>
            </div>
          ...
          {filteredAds.length > 0 ? (
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-center gap-4 pt-3 border-t border-border">
                        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                          ...
                          <span className="font-medium text-foreground">{(ad.impressions || 0).toLocaleString()}</span> impressions
                        </div>
                        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                          <MousePointerClick className="w-3.5 h-3.5" />
                          <span className="font-medium text-foreground">{(ad.clicks || 0).toLocaleString()}</span> clicks
                        </div>
                        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            ...
            <div className="text-center py-12 rounded-xl border border-dashed border-border">
              ...
              <p className="text-foreground font-medium">
                {filter === "all" ? "No sponsored posts yet" : `No ${filter} ads`}
              </p>
              <p className="text-sm text-muted-foreground mt-1">
                {filter === "all" ? "Create one to start reaching your audience." : "Try a different filter."}
              </p>
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="credits" className="space-y-6">
          ...
          <div className="rounded-lg bg-card shadow-sm p-5 space-y-3">
            <h3 className="text-sm font-bold text-foreground">How Credits Work</h3>
            <div className="space-y-2 text-sm text-muted-foreground">
              <p>• Use credits to create sponsored posts that appear in everyone's feed.</p>
              <p>• Boost marketplace listings to get more visibility and faster sales.</p>
              <p>• Target specific categories and locations for better results.</p>
              <p>• Set frequency caps to control how often users see your ads.</p>
            </div>
          </div>
        </TabsContent>
      </Tabs>

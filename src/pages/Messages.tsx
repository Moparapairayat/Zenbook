...
const Messages = () => {
  ...
  const { data: messageSearchResults } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data || []).map((m: any) => {
        ...
        let convName = "Unknown";
        if (conv) {
          if (conv.is_group) {
            convName = conv.group_name || "Group Chat";
          } else {
            const otherId = conv.participant_one === user!.id ? conv.participant_two : conv.participant_one;
            convName = profileMap.get(otherId)?.display_name || "User";
          }
        }
        return {
          ...m,
          convName,
          senderName: profileMap.get(m.sender_id)?.display_name || "User",
  ...
  const getConversationDisplayName = (conv: Conversation) => {
    ...
    return conv.otherUser?.display_name || "User";
  };
  ...
  const getConversationSubtext = (conv: Conversation) => {
    ...
    if (conv.otherUser && isOnline(conv.otherUser.user_id)) return "Active now";
    return "Offline";
  };
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {conversations?.length === 0 && (
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p className="text-xs mt-1">Click "New" or "Group" to start chatting</p>
              </div>
            )}
              ...
              <div className="flex items-center justify-between gap-2 min-w-0">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" onClick={(e) => e.stopPropagation()}>
                    ...
                    <DropdownMenuItem onClick={() => {
                      toast.info("Pinned conversation");
                    }}>
                      <Pin className="w-4 h-4 mr-2" />
                      Pin
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => {
                      toast.info("Muted conversation");
                    }}>
                      <BellOff className="w-4 h-4 mr-2" />
                      Mute
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => {
                      toast.info("Archived conversation");
                    }}>
            ...
            <div className="relative flex-1 flex h-full min-w-0 overflow-hidden">
                      ...
                      {pinnedMsgs.map((pm: any) => (
                        <p key={pm.id} className="text-xs text-muted-foreground truncate">
                          <span className="font-medium text-foreground">
                            {pm.sender_id === user?.id ? "You" : (pm.senderProfile?.display_name || "User")}:
                          </span>{" "}
                          {pm.content}
                        </p>
                      ))}
                    ...
                    <div key={msg.id} className={`flex ${isMine ? "justify-end" : "justify-start"} group/msg`}>
                      ...
                      <div className="max-w-[82%] sm:max-w-[70%]">
                        ...
                        {!isMine && activeConv.is_group && (
                          <UserProfileCard userId={msg.sender_id}>
                            <p className="text-xs text-muted-foreground mb-0.5 ml-1 hover:underline cursor-pointer inline-block">
                              {msg.senderProfile?.display_name || "User"}
                            </p>
                          </UserProfileCard>
                        )}
                        {msg.pinned && (
                          ...
                          {editingMessageId === msg.id ? (
                            <div className="flex flex-col gap-1.5">
                              ...
                              <div className="flex items-center gap-1 justify-end">
                                <button onClick={cancelEditing} className="text-[11px] px-2 py-0.5 rounded hover:bg-background/20 transition-colors">Cancel</button>
                            ...
                            <>
                              ...
                              <div className={`flex items-center gap-1 mt-1 ${isMine ? "justify-end" : ""}`}>
                                {msg.edited_at && (
                                  <span className={`text-[10px] italic ${isMine ? "text-primary-foreground/60" : "text-muted-foreground"}`}>edited</span>
                                )}
                        ...
                        {isMine && activeConv.is_group && msg.readReceipts && msg.readReceipts.length > 0 && (
                          <div className="flex items-center gap-0.5 mt-1 justify-end" title={msg.readReceipts.map((r: any) => r.profile?.display_name || "User").join(", ")}>
                            ...
                            {msg.readReceipts.slice(0, 5).map((r: any) => (
                              <img
                                ...
                                title={r.profile?.display_name || "User"}
                ...
                <div className="px-4 py-2 text-xs text-muted-foreground">
                  <span className="inline-flex items-center gap-1">
                    ...
                    {activeConv?.is_group
                      ...
                      : `${activeConv?.otherUser?.display_name || "User"} is typing...`}
                  </span>
                </div>
              )}
              ...
              {activeConv.is_group && showGroupSettings && (
                <GroupChatSettings
                  ...
                  groupName={activeConv.group_name || "Group Chat"}

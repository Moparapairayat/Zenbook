...
const Profile = () => {
  ...
  const coverOffsetY = dragOffsetY ?? profile?.cover_photo_offset_y ?? 50;

  const handleRepositionStart = () => {
    setRepositioning(true);
    setDragOffsetY(profile?.cover_photo_offset_y ?? 50);
  };

  const handleRepositionSave = async () => {
    if (!user || dragOffsetY === null) return;
    const { error } = await supabase
      .from("profiles")
      .update({ cover_photo_offset_y: Math.round(dragOffsetY) })
      .eq("user_id", user.id);
    if (error) { toast.error(error.message); return; }
    toast.success("Cover photo repositioned!");
  ...
  const handleDragMove = useCallback((e: React.MouseEvent | React.TouchEvent) => {
    if (!repositioning || dragStartY === null) return;
    const clientY = "touches" in e ? e.touches[0].clientY : e.clientY;
    const delta = dragStartY - clientY;
    setDragOffsetY((prev) => Math.max(0, Math.min(100, (prev ?? 50) + delta * 0.3)));
    setDragStartY(clientY);
  }, [repositioning, dragStartY]);
  ...
  return (
    <AppPageShell as="div" contentClassName="!max-w-[680px]">
      ...
      {/* Cover Photo */}
      <div
        className={`relative group/cover h-[200px] sm:h-[320px] rounded-lg overflow-hidden bg-secondary ${repositioning ? "cursor-grab active:cursor-grabbing" : ""}`}
        onMouseDown={repositioning ? (e) => setDragStartY(e.clientY) : undefined}
        onMouseMove={repositioning ? handleDragMove : undefined}
        onMouseUp={repositioning ? () => setDragStartY(null) : undefined}
        onMouseLeave={repositioning ? () => setDragStartY(null) : undefined}
        onTouchStart={repositioning ? (e) => setDragStartY(e.touches[0].clientY) : undefined}
        onTouchMove={repositioning ? handleDragMove : undefined}
        onTouchEnd={repositioning ? () => setDragStartY(null) : undefined}
      >
        {profile.cover_photo_url ? (
          <img
            src={profile.cover_photo_url}
            alt="Cover"
            className={`w-full h-full object-cover select-none ${!repositioning && !isOwn ? "cursor-pointer" : !repositioning ? "cursor-pointer" : ""}`}
            style={{ objectPosition: `center ${coverOffsetY}%` }}
            draggable={false}
            onClick={() => {
              if (!repositioning && profile.cover_photo_url) {
        ...
        {repositioning && (
          <div className="absolute inset-0 bg-black/30 flex flex-col items-center justify-center gap-3 z-10">
            <p className="text-white text-sm font-medium drop-shadow-lg">Drag to reposition</p>
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="px-4 py-1.5 bg-card/90 text-foreground rounded-lg text-sm font-semibold hover:bg-card transition-colors"
              >
                Cancel
              </button>
            </div>
        ...
        {isOwn && !repositioning && (
          <div className="absolute bottom-3 right-3 flex items-center gap-2">
            ...
            >
              ...
              {profile.cover_photo_url ? "Change photo" : "Add cover photo"}
            </button>
          </div>
        )}
      </div>

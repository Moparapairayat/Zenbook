...
const FILTER_OPTIONS = [
  ...
  { value: "like", label: "Likes", icon: ThumbsUp },
  { value: "comment", label: "Comments", icon: MessageCircle },
  { value: "groups", label: "Groups", icon: Users },
  { value: "pages", label: "Pages", icon: Flag },
  { value: "friend_request", label: "Requests", icon: UserPlus },
  { value: "memory", label: "Memories", icon: Clock },
...
const Notifications = () => {
  ...
  const clearAllNotifications = async () => {
    ...
    if (error) {
      ...
      toast({ description: "Failed to clear notifications", variant: "destructive" });
    } else {
      queryClient.invalidateQueries({ queryKey: ["notifications"] });
  ...
  const getTypeLabel = (type: string) => {
    switch (type) {
      ...
      case "group_post_like": return "Group Like";
      case "group_post_comment": return "Group Comment";
      case "friend_request": return "Friend Request";
      case "memory": return "Memory";
      case "event": return "Event";
      case "page_follow": return "Page Follow";
      case "page_post_like": return "Page Reaction";
      case "page_post_comment": return "Page Comment";
    ...
    return (
      <>
        {today.length > 0 && (
          <div>
            <div className="px-4 py-2.5 bg-muted/40 border-b border-border">
              <span className="text-[11px] font-bold uppercase tracking-widest text-primary">Today</span>
            </div>
        ...
        {thisWeek.length > 0 && (
          <div>
            <div className="px-4 py-2.5 bg-muted/40 border-b border-border">
              <span className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground">This Week</span>
            </div>
        ...
        {earlier.length > 0 && (
          <div>
            <div className="px-4 py-2.5 bg-muted/40 border-b border-border">
              <span className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground">Earlier</span>
            </div>
      ...
      <AppPageShell>
        ...
        {showClearConfirm && (
          <div className="mb-4 p-4 rounded-xl bg-destructive/5 border border-destructive/20 flex items-center justify-between animate-fade-in">
            <p className="text-sm text-foreground font-medium">Delete all notifications? This can't be undone.</p>
            <div className="flex gap-2">
              <Button variant="secondary" size="sm" onClick={() => setShowClearConfirm(false)}>Cancel</Button>
              <Button variant="destructive" size="sm" onClick={clearAllNotifications}>Delete all</Button>
            </div>
          </div>
        )}
        ...
        {!isLoading && notifications.length === 0 && (
          <div className="bg-card rounded-2xl border border-border p-14 text-center">
            ...
            <h3 className="text-lg font-bold text-foreground mb-2">
              {filter === "all" ? "No notifications yet" : `No ${FILTER_OPTIONS.find(o => o.value === filter)?.label.toLowerCase()} notifications`}
            </h3>
            <p className="text-sm text-muted-foreground max-w-xs mx-auto leading-relaxed">
              {filter === "all"
                ? "When someone interacts with your posts or sends a friend request, you'll see it here."
            ...
            {filter !== "groups"
              ...
              : (() => {
                  ...
                  const sectionLabels: Record<string, string> = {
                    group_post_like: "Group Likes",
                    group_post_comment: "Group Comments",
                    event: "Events",
                    event_digest: "Weekly Digest",
                  };

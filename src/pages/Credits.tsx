...
const Credits = () => {
  ...
  const claimFreeCredits = async () => {
    ...
    toast.success("You received 50 free promotion credits!");
  ...
  const getTypeBadge = (type: string, amount: number) => {
    const config: Record<string, { label: string; variant: "default" | "secondary" | "destructive" | "outline" }> = {
      ...
      gift_received: { label: "Gift Received", variant: "default" },
      gift_sent: { label: "Gift Sent", variant: "secondary" },
      spend: { label: "Spent", variant: "destructive" },
      promotion: { label: "Promotion", variant: "destructive" },
    };
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-destructive">-{totalSpent}</p>
                </div>
        ...
        <div className="flex flex-wrap gap-3">
          {balance === 0 && (
            <Button onClick={claimFreeCredits} variant="outline" className="gap-2">
              <Gift className="w-4 h-4" /> Claim 50 Free Credits
            </Button>
          )}
          <Button onClick={() => setGiftOpen(true)} className="gap-2">
            <Gift className="w-4 h-4" /> Gift Credits to Friend
          </Button>
        </div>

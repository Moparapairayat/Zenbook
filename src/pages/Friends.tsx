...
const Friends = () => {
  ...
  const tabs = [
    { key: "friends" as const, label: "All Friends", count: acceptedFriends.length },
    ...
    <AppPageShell>
      ...
      {tab === "friends" && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            return (
              <div key={f.id} className="bg-card rounded-lg shadow-sm p-4 flex items-center gap-3">
                ...
                <div className="flex-1">
                  <p className="font-semibold text-foreground">{friend?.display_name || "User"}</p>
                </div>
          ...
          {acceptedFriends.length === 0 && (
            <p className="text-muted-foreground col-span-2 text-center py-8">No friends yet. Find people to connect with!</p>
          )}
        </div>
      )}
      ...
      {tab === "requests" && (
        <div className="space-y-4">
          {pendingRequests.length > 0 && <h3 className="font-semibold text-foreground">Incoming Requests</h3>}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {pendingRequests.map((f: any) => (
              <div key={f.id} className="bg-card rounded-lg shadow-sm p-4 flex items-center gap-3">
                ...
                <div className="flex-1">
                  <p className="font-semibold text-foreground">{f.requester?.display_name || "User"}</p>
                </div>
                <div className="flex gap-2">
                  <Button size="sm" onClick={() => respondRequest(f.id, true)}>Confirm</Button>
                  <Button variant="secondary" size="sm" onClick={() => respondRequest(f.id, false)}>Delete</Button>
                </div>
              </div>
            ))}
          </div>
          ...
          {allProfiles?.map((p: any) => {
            ...
            return (
              <div key={p.id} className="bg-card rounded-lg shadow-sm p-4 flex items-center gap-3">
                ...
                <div className="flex-1">
                  <p className="font-semibold text-foreground">{p.display_name || "User"}</p>
                </div>
                {isFriend ? (

...
const CATEGORIES = ["All", "Gaming", "Music", "Sports", "News", "Education", "Entertainment", "Tech", "Cooking", "Travel"];
  ...
  const { data: videos = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data || []).map((v: any) => ({
        ...v,
        profile: profileMap.get(v.user_id) || { display_name: "User", avatar_url: null },
      }));
    },
    enabled: !!user,
  });
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  {CATEGORIES.map((cat) => (
                    <DropdownMenuItem key={cat} onClick={() => setActiveCategory(cat)} className={activeCategory === cat ? "bg-primary/10 text-primary font-medium" : ""}>
                      {cat}
                    </DropdownMenuItem>
                  ))}
                </DropdownMenuContent>
            ...
            {videos.length === 0 ? (
              <div className="text-center py-20 text-muted-foreground">
                ...
                <p className="text-sm mt-1">Be the first to upload a video!</p>
                <Button onClick={() => setShowUpload(true)} className="mt-4">Upload Video</Button>
              </div>
            ) : (

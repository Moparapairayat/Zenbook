...
const Groups = () => {
  ...
  const handleJoin = async (groupId: string) => {
    ...
    try {
      ...
      toast.success(status === "pending" ? "Join request sent!" : "Joined group!");
    } catch (err: any) {
        ...
        {displayGroups.length > 0 ? (
          ...
          <div className="bg-card rounded-xl shadow-sm p-12 text-center">
            ...
            <p className="text-muted-foreground font-medium">
              {tab === "your" ? "You haven't created any groups yet" : tab === "my" ? "You haven't joined any groups yet" : "No groups to discover"}
            </p>
            {(tab === "your" || tab === "my") && (
              <button
              ...
              >
                {tab === "your" ? "Create a group →" : "Discover groups →"}
              </button>
            )}
          </div>
        )}

    ...
    return (
      <AppPageShell>
        <div className="py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">This event may have been deleted or you don't have access.</p>
    ...
    <AppPageShell>
        ...
        <div className="rounded-lg bg-card shadow-sm p-5 sm:p-6">
          ...
          <div className="flex items-center gap-2 flex-wrap mb-3">
            ...
            {isPast && (
              <span className="text-xs font-medium text-muted-foreground bg-muted px-2.5 py-1 rounded-full">Past event</span>
            )}
          </div>
          ...
          <div className="mt-4 flex items-center gap-3 text-sm text-muted-foreground">
            {event.profile && (
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs">Created by <span className="font-medium text-foreground">{event.profile.display_name || "Unknown"}</span></span>
              </div>
            )}
          </div>
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                ...
                <span>
                  {event.attendees.going.length > 0 && (
                    <span className="font-medium text-foreground">{event.attendees.going.length} going</span>
                  )}
                  {event.attendees.going.length > 0 && event.attendees.interested.length > 0 && " · "}
                  {event.attendees.interested.length > 0 && (
                    <span>{event.attendees.interested.length} interested</span>
                  )}
                </span>
                <span className="ml-auto text-[10px] text-muted-foreground/70">View all →</span>
              </div>
              {/* Stacked avatars */}
        ...
        <div className="rounded-lg bg-card shadow-sm p-5 sm:p-6 mt-4">
          <h2 className="text-sm font-semibold text-foreground mb-1">Discussion</h2>

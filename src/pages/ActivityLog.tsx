...
const ActivityLog = () => {
    ...
    queryFn: async () => {
      ...
      if (filter === "all" || filter === "posts") {
        ...
        posts?.forEach((p) =>
          results.push({
            id: `post-${p.id}`,
            type: "post",
            description: `Created a post: "${p.content.slice(0, 80)}${p.content.length > 80 ? "…" : ""}"`,
      ...
      if (filter === "all" || filter === "comments") {
        ...
        comments?.forEach((c) =>
          results.push({
            id: `comment-${c.id}`,
            type: "comment",
            description: `Commented: "${c.content.slice(0, 80)}${c.content.length > 80 ? "…" : ""}"`,
      ...
      if (filter === "all" || filter === "likes") {
        ...
        likes?.forEach((l) =>
          results.push({
            id: `like-${l.id}`,
            type: "like",
            description: `Reacted to a post with ${l.reaction_type}`,
      ...
      if (filter === "all" || filter === "events") {
        const { data: rsvps } = await supabase
          ...
          .limit(20);
        rsvps?.forEach((r) => {
          const eventTitle = (r.group_events as any)?.title || "an event";
          results.push({
            id: `rsvp-${r.id}`,
            type: "event",
            description: `RSVP'd "${r.status}" to ${eventTitle}`,
      ...
      if (filter === "all" || filter === "marketplace") {
        ...
        listings?.forEach((l) =>
          results.push({
            id: `listing-${l.id}`,
            type: "marketplace",
            description: `Listed "${l.title}" for $${l.price}`,
            timestamp: l.created_at,
            icon: ShoppingBag,
      ...
      if (filter === "all" || filter === "groups") {
        ...
        groupPosts?.forEach((gp) => {
          const groupName = (gp.groups as any)?.name || "a group";
          results.push({
            id: `gpost-${gp.id}`,
            type: "group",
            description: `Posted in ${groupName}: "${gp.content.slice(0, 60)}${gp.content.length > 60 ? "…" : ""}"`,
  ...
  const groupByDate = (items: Activity[]) => {
    ...
    items.forEach((item) => {
      ...
      const label = format(new Date(item.timestamp), "EEEE, MMMM d, yyyy");
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <h1 className="text-xl font-bold text-foreground leading-tight">Activity Log</h1>
          </div>
        </div>
        <DropdownMenu>

    ...
    <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
      ...
      <div className="px-4 py-2 border-t border-border flex items-center gap-4">
        ...
        >
          ...
          {showReactions && (
            <div className="absolute bottom-full left-0 mb-2 bg-card border border-border rounded-full shadow-lg px-2 py-1.5 flex items-center gap-1 z-50 animate-in fade-in slide-in-from-bottom-2 duration-200">
              {REACTIONS.map((r) => (
                <button
                  key={r.type}
...
const PageDetail = () => {
  ...
  const handlePost = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(scheduled_at ? "Post scheduled!" : "Post published!");
    } catch {
  ...
  const handleReactPost = async (postId: string, reactionType: string) => {
    ...
    try {
      if (userLike && userLike.reaction_type === reactionType) {
        ...
        if (page.created_by !== user.id) {
          const reactionLabel = reactionType === "like" ? "liked" : `reacted ${reactionType} to`;
          await supabase.from("notifications").insert({
            user_id: page.created_by,
            actor_id: user.id,
            type: "page_post_like",
            message: `${reactionLabel} a post on "${page.name}"`,
            reference_id: postId,
          });
        }
      }
        ...
        >
          {isOwner && (
            <label className="absolute bottom-3 right-3 flex items-center gap-2 px-3 py-2 rounded-lg bg-black/50 text-white text-xs font-medium cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/70">
              ...
              {uploadingCover ? "Uploading..." : page.cover_photo_url ? "Change cover" : "Add cover photo"}
        ...
        <div className="-mt-10 sm:-mt-12">
          <div className="flex flex-col sm:flex-row items-start sm:items-end gap-3 sm:gap-4 overflow-hidden">
            ...
            <div className="flex-1 min-w-0 overflow-hidden">
              ...
              <p className="text-sm text-muted-foreground truncate">{categoryLabel} · {followerCount} {followerCount === 1 ? "follower" : "followers"}</p>
            </div>
              ...
              {isOwner && (
                <>
                  <Button variant="outline" size="sm" onClick={() => setShowBoostPageModal(true)}>
                    <Rocket className="w-4 h-4" /> Boost Page
                  </Button>
          ...
          <div className="mt-6">
            <ScheduledPostsCalendar
              ...
              onClickSlot={(date, hour) => {
                ...
                setActiveTab("posts");
                toast.info(`Scheduling for ${format(date, "MMM d")} at ${hour > 12 ? hour - 12 : hour || 12}:00 ${hour >= 12 ? "PM" : "AM"} — write your post below`);
              }}
            />
          </div>
        ) : (
              ...
              {editingAbout ? (
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button variant="secondary" size="sm" onClick={() => setEditingAbout(false)}>Cancel</Button>
                  </div>
                </div>
              ) : (
                <>
                  ...
                  {isOwner && (
                    ...
                    >
                      {page.about ? "Edit about" : "Add about info"}
                    </Button>
                  )}
                </>
              )}
                    ...
                    <Popover>
                      ...
                      <PopoverContent className="w-auto p-0" align="start">
                        <div className="p-3 border-b border-border">
                          <p className="text-sm font-semibold text-foreground mb-2">Schedule post</p>
            ...
            {isOwner && scheduledPosts.length > 0 && (
              <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
                <div className="p-4 border-b border-border flex items-center gap-2">
                  <Clock className="w-4 h-4 text-primary" />
                  <h3 className="text-sm font-semibold text-foreground">Scheduled Posts ({scheduledPosts.length})</h3>
                </div>
      ...
      <AlertDialog open={!!publishConfirmPostId} onOpenChange={(open) => { if (!open) setPublishConfirmPostId(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will immediately publish the scheduled post. It will appear in your page's feed right away and your followers will be notified.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

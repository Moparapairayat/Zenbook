...
const fetchPostsPage = async ({ pageParam, myGroupIds, followedPageIds }: { pageParam: number; myGroupIds: string[]; followedPageIds: string[] }) => {
...
const Index = () => {
  ...
  const {
    ...
    queryKey: ["posts", myGroupIds, followedPageIds],
    queryFn: ({ pageParam }) => fetchPostsPage({ pageParam, myGroupIds, followedPageIds }),
    initialPageParam: 0,
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 z-40 w-11 h-11 rounded-full bg-primary text-primary-foreground shadow-lg flex items-center justify-center transition-all duration-300 hover:opacity-90 ${

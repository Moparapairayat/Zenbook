...
const navItems: { id: AdminSection; label: string; icon: React.ElementType }[] = [
  ...
  { id: "sellers", label: "Verified Sellers", icon: BadgeCheck },
  { id: "verification", label: "Verification Requests", icon: ShieldCheck },
  { id: "cms", label: "CMS Pages", icon: FileText },
  { id: "advertisements", label: "Site Ads", icon: Megaphone },
  { id: "user-ads", label: "User Ads", icon: Megaphone },
  { id: "design", label: "Site Design", icon: Palette },
  { id: "custom-code", label: "Custom JS/CSS", icon: Code2 },
  { id: "emails", label: "Manage Emails", icon: Mail },
  { id: "send-email", label: "Send Email", icon: Send },
  { id: "push-notifications", label: "Push Notifications", icon: Bell },
  { id: "genders", label: "Manage Genders", icon: Users },
  { id: "online-users", label: "Online Users", icon: Wifi },
  { id: "video-audio", label: "Video & Audio", icon: Video },
  { id: "file-upload", label: "File Upload", icon: Upload },
  { id: "email-sms", label: "Email & SMS", icon: Phone },
...
const navGroups: NavGroup[] = [
  { label: "Dashboard", icon: LayoutDashboard, items: ["overview"] },
  { label: "Content Management", icon: FolderOpen, items: ["users", "posts", "groups", "pages", "events", "listings"] },
  { label: "Moderation & Safety", icon: Shield, items: ["reports", "fraud", "sellers", "verification"] },
  { label: "Content & Ads", icon: Megaphone, items: ["cms", "advertisements", "user-ads"] },
  { label: "Appearance", icon: Palette, items: ["design", "custom-code"] },
  { label: "Communication", icon: Mail, items: ["emails", "send-email", "push-notifications", "email-sms"] },
  { label: "System Settings", icon: Wrench, items: ["genders", "online-users", "video-audio", "file-upload", "settings"] },
];
...
const Moderation = () => {
  ...
  useEffect(() => {
    ...
    const fetchReports = async () => {
      ...
      let query = (supabase as any)
        ...
        .select("*, listing:listings(id, title, price, status, user_id), reporter:profiles!listing_reports_reporter_id_fkey(display_name)")
  ...
  const handleResolveSignal = async (resolve: boolean) => {
    ...
    toast.success(resolve ? "Signal resolved" : "Signal reopened");
  ...
  const handleRemoveFromSignal = async () => {
    ...
    await (supabase as any)
      ...
      .eq("id", selectedSignal.id);
    toast.success("Listing removed and signal resolved");
  ...
  if (!isAdmin) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-muted-foreground">You don't have permission to access the moderation dashboard.</p>
  ...
  const reportStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "warned": return <Badge className="bg-amber-500/15 text-amber-600 border-amber-500/30">Warned</Badge>;
      case "removed": return <Badge className="bg-red-600/15 text-red-600 border-red-600/30">Removed</Badge>;
  ...
  const severityBadge = (severity: string) => {
    switch (severity) {
      ...
      case "medium": return <Badge className="bg-amber-500/15 text-amber-600 border-amber-500/30">Medium</Badge>;
            ...
            <div className="p-4 lg:p-6 max-w-5xl">
              ...
              {activeSection === "overview" && (
                <div className="space-y-6">
                  {/* Platform Stats */}
                  <div>
                    <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Platform Overview</h3>
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                      {[
                        { label: "Users", value: platformStats.users, icon: Users, section: "users" as AdminSection },
                        { label: "Groups", value: platformStats.groups, icon: Users, section: "groups" as AdminSection },
                        { label: "Pages", value: platformStats.pages, icon: Globe2, section: "pages" as AdminSection },
                        { label: "Events", value: platformStats.events, icon: CalendarDays, section: "events" as AdminSection },
                        { label: "Listings", value: platformStats.listings, icon: Package, section: "listings" as AdminSection },
                        { label: "Posts", value: platformStats.posts, icon: MessageSquare, section: undefined },
                      ].map((stat) => (
                        <Card
                          key={stat.label}
                  ...
                  {/* Moderation Stats */}
                  <div>
                    <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Moderation</h3>
                      ...
                      <Card>
                        <CardContent className="p-4 flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">Resolved Signals</p>
                          </div>
                        </CardContent>
                      </Card>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                      {[
                        { label: "Website Logo", desc: "Change your logo from Site Design", icon: Palette, section: "design" as AdminSection },
                        { label: "Site Design", desc: "Customize colors and branding", icon: Palette, section: "design" as AdminSection },
                        { label: "Settings", desc: "Configure platform settings", icon: Settings, section: "settings" as AdminSection },
                        { label: "Custom Code", desc: "Add custom JS/CSS", icon: Code2, section: "custom-code" as AdminSection },
                        { label: "User Management", desc: "Manage users and roles", icon: Users, section: "users" as AdminSection },
                        { label: "CMS Pages", desc: "Manage content pages", icon: Globe2, section: "cms" as AdminSection },
                        { label: "Verification", desc: "Review verification requests", icon: ShieldCheck, section: "verification" as AdminSection },
                        { label: "Email & SMS", desc: "Configure email and SMS", icon: Mail, section: "email-sms" as AdminSection },
                        { label: "Push Notifications", desc: "Configure push notifications", icon: Bell, section: "push-notifications" as AdminSection },
                        { label: "Advertisements", desc: "Manage site advertisements", icon: Megaphone, section: "advertisements" as AdminSection },
                        { label: "File Upload", desc: "Configure file upload settings", icon: Package, section: "file-upload" as AdminSection },
                        { label: "Video & Audio", desc: "Configure media settings", icon: Video, section: "video-audio" as AdminSection },
                      ].map((action) => (
                        <Card
                          key={action.label}
                    ...
                    <CardContent>
                      {reports.length === 0 && fraudSignals.length === 0 ? (
                        <div className="text-center py-8 text-muted-foreground">
                          ...
                          <p className="text-sm font-medium">All clear! No recent activity.</p>
                        </div>
                      ) : (
                  ...
                  <Card>
                    <CardHeader>
                      <CardTitle className="text-lg">
                        {filter === "pending" ? "Pending Reports" : filter === "reviewed" ? "Reviewed Reports" : "All Reports"}
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                      {loading ? (
              ...
              {activeSection === "fraud" && (
                <div className="space-y-6">
                  ...
                  <Card>
                    <CardHeader>
                      <CardTitle className="text-lg">
                        {fraudFilter === "unresolved" ? "Unresolved Fraud Signals" : fraudFilter === "resolved" ? "Resolved Signals" : "All Fraud Signals"}
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                      {fraudLoading ? (
                        ...
                        <div className="text-center py-12 text-muted-foreground">
                          <CheckCircle className="w-12 h-12 mx-auto mb-3 text-muted-foreground/50" />
                          <p className="font-medium">No fraud signals detected</p>
                          <p className="text-sm">Auto-detection is working. Nothing flagged yet.</p>
                        </div>
                      ) : (
                        <Table>
                          <TableHeader>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{selectedReport.listing?.title || "Unknown"}</p>
                  </div>
                ...
                {selectedReport.listing && (
                  <div className="flex gap-4 text-sm">
                    <span className="text-muted-foreground">Price: <span className="text-foreground font-medium">${selectedReport.listing.price}</span></span>
                ...
                <>
                  ...
                  <div className="flex gap-2 justify-end">
                    ...
                    <Button variant="secondary" size="sm" onClick={() => handleAction("warn")} disabled={processing}
                      ...
                      <AlertTriangle className="w-4 h-4 mr-1" /> Warn Seller
                    </Button>
                    <Button variant="destructive" size="sm" onClick={() => handleAction("remove")} disabled={processing}>
                      <Trash2 className="w-4 h-4 mr-1" /> Remove Listing
                    </Button>
                  </div>
                </>
              )}
        ...
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Bot className="w-5 h-5" /> Fraud Signal Details
            </DialogTitle>
          </DialogHeader>
          {selectedSignal && (
            <div className="space-y-4">
              <div className="space-y-2">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-sm font-medium text-foreground">Listing</p>
                    <p className="text-sm text-muted-foreground">{selectedSignal.listing?.title || "Unknown"}</p>
                  </div>
                ...
                {selectedSignal.listing && (
                  <div className="flex gap-4 text-sm">
                    <span className="text-muted-foreground">Price: <span className="text-foreground font-medium">${selectedSignal.listing.price}</span></span>

...
import { ShieldCheck, CheckCircle, XCircle, Clock, User, Store, FileText, ExternalLink } from "lucide-react";
...
const AdminVerificationRequests = () => {
  ...
  const statusBadge = (status: string) => {
    switch (status) {
      case "pending":
        return <Badge variant="outline" className="text-yellow-600 border-yellow-400"><Clock className="w-3 h-3 mr-1" />Pending</Badge>;
      case "approved":
        return <Badge variant="outline" className="text-green-600 border-green-400"><CheckCircle className="w-3 h-3 mr-1" />Approved</Badge>;
      case "rejected":
        return <Badge variant="outline" className="text-red-600 border-red-400"><XCircle className="w-3 h-3 mr-1" />Rejected</Badge>;
      default:
  ...
  const typeBadge = (type: string) => {
    if (type === "seller") return <Badge variant="secondary"><Store className="w-3 h-3 mr-1" />Seller</Badge>;
    return <Badge variant="secondary"><User className="w-3 h-3 mr-1" />Profile</Badge>;
  };
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Review and manage profile and seller verification requests.
        </p>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value={activeTab} className="mt-4 space-y-2">
          {isLoading && <p className="text-sm text-muted-foreground text-center py-8">Loading requests...</p>}

          {!isLoading && filtered.length === 0 && (
            <p className="text-sm text-muted-foreground text-center py-8">No {activeTab !== "all" ? activeTab : ""} verification requests.</p>
          )}
              ...
              <Card key={req.id} className="cursor-pointer hover:bg-muted/30 transition-colors" onClick={() => { setSelectedRequest(req); setAdminNote(req.admin_note || ""); }}>
                <CardContent className="flex items-center gap-3 py-3 px-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate">{req.full_name || profile?.display_name || "Unknown User"}</p>
                    <p className="text-xs text-muted-foreground truncate">{req.reason || "No reason provided"}</p>
                  </div>
            ...
            return (
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold">{selectedRequest.full_name || profile?.display_name || "Unknown"}</p>
                ...
                {selectedRequest.status !== "pending" && selectedRequest.reviewed_at && (
                  <p className="text-xs text-muted-foreground">
                    Reviewed on {format(new Date(selectedRequest.reviewed_at), "PPp")}
                  </p>
                )}
              </div>
            );

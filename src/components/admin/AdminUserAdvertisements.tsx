...
import {
  Megaphone, Search, Eye, Trash2, Ban, CheckCircle, XCircle,
  ExternalLink, BarChart3, MousePointerClick, DollarSign, Calendar,
  Filter, Image, TrendingUp,
} from "lucide-react";
...
const AdminUserAdvertisements = () => {
  ...
  const [adToReject, setAdToReject] = useState<any>(null);
  ...
  const handleReject = async () => {
    ...
    if (error) {
      ...
      toast.success("Ad rejected and deactivated");
      queryClient.invalidateQueries({ queryKey: ["admin-user-ads"] });
    }
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Review, approve, and manage user-created sponsored posts and ad campaigns.
        </p>
      </div>
        ...
        <Card>
          <CardContent className="p-3 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground">Credits Spent</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            ...
            placeholder="Search by content or advertiser..."
      ...
      {/* Table */}
      {isLoading ? (
        <div className="text-center py-12 text-muted-foreground text-sm">Loading advertisements...</div>
      ) : filteredAds.length === 0 ? (
        <div className="text-center py-16 rounded-xl border border-dashed border-border">
          <Megaphone className="w-12 h-12 mx-auto mb-3 text-muted-foreground/30" />
          <p className="text-foreground font-medium">No advertisements found</p>
          <p className="text-sm text-muted-foreground mt-1">
            {search || statusFilter !== "all" ? "Try adjusting your filters." : "No users have created ads yet."}
          </p>
        </div>
      ) : (
        <Card>
            ...
            return (
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-foreground">{profile?.display_name || "Unknown User"}</p>
                    <p className="text-[10px] text-muted-foreground">Created {format(new Date(selectedAd.created_at), "PPP")}</p>
                  </div>
                ...
                <div className="space-y-1.5">
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Ad Content</p>
                  <p className="text-sm text-foreground bg-muted/50 rounded-lg p-3">{selectedAd.content}</p>
                </div>

                {selectedAd.image_url && (
                  <div className="space-y-1.5">
                    <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Image</p>
                    <img src={selectedAd.image_url} alt="Ad" className="rounded-lg max-h-40 object-cover w-full" />
                  </div>
                )}
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {selectedAd.target_category && (
                      <Badge variant="outline" className="text-[10px]">Category: {selectedAd.target_category}</Badge>
                    )}
                    {selectedAd.target_location && (
                      <Badge variant="outline" className="text-[10px]">Location: {selectedAd.target_location}</Badge>
                    )}
                    {selectedAd.target_gender && (
                      <Badge variant="outline" className="text-[10px]">Gender: {selectedAd.target_gender}</Badge>
                    )}
                    {selectedAd.target_age_min && (
                      <Badge variant="outline" className="text-[10px]">Age: {selectedAd.target_age_min}–{selectedAd.target_age_max || "∞"}</Badge>
                    )}
                    {!selectedAd.target_category && !selectedAd.target_location && !selectedAd.target_gender && !selectedAd.target_age_min && (
                      <span className="text-xs text-muted-foreground">No targeting — shown to all users</span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  {format(new Date(selectedAd.start_date), "PPP")} — {format(new Date(selectedAd.end_date), "PPP")}
                  <span className="ml-1">
                    (Freq cap: {selectedAd.frequency_cap || "none"})
                  </span>
                </div>
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  >
                    ...
                    {selectedAd.is_active ? "Deactivate Ad" : "Activate Ad"}
                  </Button>
                  <Button
                    variant="outline"
                    size="sm"

...
const AdminPagesManagement = () => {
  ...
  const fetchPages = async () => {
    ...
    if (pagesData) {
      ...
      setPages(pagesData.map((p: any) => ({ ...p, creator_name: profileMap.get(p.created_by) || "Unknown", follower_count: countMap.get(p.id) || 0 })));
    }
    setLoading(false);
  };
  ...
  const handleSavePage = async () => {
    ...
    toast.success("Page updated"); setSaving(false); setEditingPage(null); fetchPages();
  };

  const handleDeletePage = async (pageId: string) => {
    setDeleting(true);
    await supabase.from("pages").delete().eq("id", pageId);
    toast.success("Page deleted"); setConfirmDelete(null); setDeleting(false); fetchPages();
  };
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div><p className="text-2xl font-bold text-foreground">{pages.length}</p><p className="text-xs text-muted-foreground">Total Pages</p></div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center"><FileText className="w-5 h-5 text-primary" /></div>
            <div><p className="text-2xl font-bold text-foreground">{pages.reduce((sum, p) => sum + (p.follower_count || 0), 0)}</p><p className="text-xs text-muted-foreground">Total Followers</p></div>
          </CardContent>
        </Card>
      </div>
      ...
      <Dialog open={!!editingPage} onOpenChange={(open) => !open && setEditingPage(null)}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            ...
            <Button onClick={handleSavePage} disabled={!editName.trim() || saving} className="w-full">{saving ? "Saving..." : "Save Changes"}</Button>
          </div>
        </DialogContent>
      </Dialog>

      <ConfirmDialog open={!!confirmDelete} onOpenChange={(open) => !open && setConfirmDelete(null)} title="Delete Page" description="Are you sure you want to delete this page permanently? All posts and followers will be removed." confirmLabel="Delete" onConfirm={() => confirmDelete && handleDeletePage(confirmDelete)} loading={deleting} />
      <ConfirmDialog open={confirmBulkDelete} onOpenChange={setConfirmBulkDelete} title="Delete Selected Pages" description={`Are you sure you want to delete ${selected.size} page(s)? This cannot be undone.`} confirmLabel={`Delete ${selected.size} Pages`} onConfirm={handleBulkDelete} loading={deleting} />
    </div>
  );
};

...
const AdminUserManagement = () => {
  ...
  const handleBulkDelete = async () => {
    ...
    if (error) {
      ...
      toast.success(`${ids.length} user profile(s) deleted`);
      setSelected(new Set());
    ...
    <>
          ...
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Block Records</p>
              </div>
            </CardContent>
          </Card>
        </div>
                      ...
                      >
                        ...
                        <TableCell>
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-medium text-sm whitespace-nowrap">{user.display_name || "Unknown"}</span>
                          </div>
                        </TableCell>
                        <TableCell>
      ...
      <Dialog open={!!selectedUser && !confirmDelete} onOpenChange={(o) => { if (!o) { setSelectedUser(null); setEditMode(false); } }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{editMode ? "Edit User" : "Manage User"}</DialogTitle>
          </DialogHeader>
          {selectedUser && (
            <div className="space-y-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-foreground">{selectedUser.display_name || "Unknown"}</p>
              ...
              {editMode ? (
                ...
                <>
                  ...
                  >
                    <Gift className="w-3.5 h-3.5 text-amber-500" /> Send Gift Credits
                  </Button>
                  ...
                  >
                    <Trash2 className="w-3.5 h-3.5 mr-1" /> Delete User Profile
                  </Button>
                </>
              )}
            </div>
      ...
      <ConfirmDialog
        ...
        description={`Are you sure you want to delete "${selectedUser?.display_name || "this user"}"'s profile? This will also remove all their roles. This action cannot be undone.`}
        confirmLabel="Delete"
        variant="destructive"
      ...
      <ConfirmDialog
        ...
        description={`Are you sure you want to delete ${selected.size} user profile(s)? This will also remove all their roles. This action cannot be undone.`}
      ...
      {selectedUser && (
        <GiftCreditsModal
          ...
          recipientName={selectedUser.display_name || "User"}

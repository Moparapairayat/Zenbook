...
const AdminEventsManagement = () => {
  ...
  const handleDeleteEvent = async (eventId: string) => {
    ...
    toast.success("Event deleted"); setConfirmDelete(null); setDeleting(false); fetchEvents();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div><p className="text-2xl font-bold text-foreground">{events.length}</p><p className="text-xs text-muted-foreground">Total Events</p></div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center"><CalendarDays className="w-5 h-5 text-primary" /></div>
            <div><p className="text-2xl font-bold text-foreground">{upcomingCount}</p><p className="text-xs text-muted-foreground">Upcoming</p></div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div><p className="text-2xl font-bold text-foreground">{pastCount}</p><p className="text-xs text-muted-foreground">Past</p></div>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative w-56">
                ...
                <Input placeholder="Search events..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 h-8 text-sm" />
              </div>
            </div>
          </div>
        </CardHeader>
                      ...
                      <TableCell>
                        <div>
                          ...
                          <div className="flex items-center gap-1 mt-0.5">
                            <span className="inline-flex items-center text-[10px] font-medium px-1.5 py-0.5 rounded-full" style={{ backgroundColor: `${catColor}20`, color: catColor }}>{event.category}</span>
      ...
      <ConfirmDialog open={!!confirmDelete} onOpenChange={(open) => !open && setConfirmDelete(null)} title="Delete Event" description="Are you sure you want to delete this event? All RSVPs will be removed." confirmLabel="Delete" onConfirm={() => confirmDelete && handleDeleteEvent(confirmDelete)} loading={deleting} />
      <ConfirmDialog open={confirmBulkDelete} onOpenChange={setConfirmBulkDelete} title="Delete Selected Events" description={`Are you sure you want to delete ${selected.size} event(s)? This cannot be undone.`} confirmLabel={`Delete ${selected.size} Events`} onConfirm={handleBulkDelete} loading={deleting} />
    </div>
  );
};

...
const AdminGroupsManagement = () => {
  ...
  const handleSaveGroup = async () => {
    ...
    toast.success("Group updated"); setSaving(false); setEditingGroup(null); fetchGroups();
  };

  const handleDeleteGroup = async (groupId: string) => {
    setDeleting(true);
    await supabase.from("groups").delete().eq("id", groupId);
    toast.success("Group deleted"); setConfirmDelete(null); setDeleting(false); fetchGroups();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div><p className="text-2xl font-bold text-foreground">{groups.filter(g => g.privacy === "public").length}</p><p className="text-xs text-muted-foreground">Public Groups</p></div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div><p className="text-2xl font-bold text-foreground">{groups.filter(g => g.privacy === "private").length}</p><p className="text-xs text-muted-foreground">Private Groups</p></div>
          </CardContent>
        </Card>
      </div>
      ...
      <BulkActionsBar
        ...
        actions={[
          { label: `Delete ${selected.size}`, onClick: () => setConfirmBulkDelete(true), variant: "destructive", icon: <Trash2 className="w-3.5 h-3.5 mr-1" /> },
        ]}
      />
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center gap-1 justify-end">
                        <Button variant="ghost" size="sm" className="h-8 text-xs" onClick={() => handleViewMembers(group)}>
                          <Eye className="w-3.5 h-3.5 mr-1" /> Members
                        </Button>
          ...
          {membersLoading ? (
            ...
            <div className="space-y-2">
              {members.filter(m => m.status === "pending").length > 0 && (
                <div className="mb-4">
                  <p className="text-xs font-semibold text-muted-foreground uppercase mb-2">Pending Requests</p>
                  {members.filter(m => m.status === "pending").map((m) => (
                    <div key={m.id} className="flex items-center justify-between p-2 rounded-lg bg-secondary/50 mb-1">
                      <span className="text-sm font-medium">{m.display_name}</span>
                      <div className="flex gap-1">
                        <Button size="sm" className="h-7 text-xs" onClick={() => handleApproveMember(m.id)}>Approve</Button>
                        <Button size="sm" variant="destructive" className="h-7 text-xs" onClick={() => handleRemoveMember(m.id)}>Reject</Button>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div key={m.id} className="flex items-center justify-between p-2 rounded-lg hover:bg-secondary/50">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <Badge variant="outline" className="text-[10px] h-4">{m.role}</Badge>
                    </div>
                  </div>
                  <div className="flex gap-1">
                    {m.role !== "admin" && <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => handleSetRole(m.id, "admin")} title="Make Admin"><Crown className="w-3.5 h-3.5 text-primary" /></Button>}
                    {m.role !== "moderator" && <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => handleSetRole(m.id, "moderator")} title="Make Moderator"><ShieldCheck className="w-3.5 h-3.5 text-amber-600" /></Button>}
                    {m.role !== "member" && <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => handleSetRole(m.id, "member")} title="Demote"><Shield className="w-3.5 h-3.5 text-muted-foreground" /></Button>}
                    <Button variant="ghost" size="sm" className="h-7 text-xs text-destructive hover:text-destructive" onClick={() => handleRemoveMember(m.id)} title="Remove"><UserMinus className="w-3.5 h-3.5" /></Button>
                  </div>
                </div>
              ))}
            </div>
      ...
      <ConfirmDialog
        ...
        title="Delete Group"
        description="Are you sure you want to delete this group? All posts and members will be removed. This cannot be undone."
        confirmLabel="Delete"
      ...
      <ConfirmDialog
        ...
        description={`Are you sure you want to delete ${selected.size} group(s)? This cannot be undone.`}
        confirmLabel={`Delete ${selected.size} Groups`}

    ...
    <div className="rounded-lg bg-card shadow-sm p-4">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-foreground">Promotion Credits</span>
        </div>
        <span className="text-lg font-bold text-foreground">{creditBalance}</span>
      </div>
      <p className="text-xs text-muted-foreground mb-3">
        Use credits to boost listings and create sponsored posts.
      </p>
      {creditBalance === 0 && (
        <Button size="sm" variant="outline" onClick={claimFreeCredits} className="w-full gap-1.5">
          <Gift className="w-3.5 h-3.5" /> Claim 50 Free Credits
        </Button>
      )}
    </div>
  );

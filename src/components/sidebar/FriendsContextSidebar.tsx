  ...
  return (
    <div className="space-y-4">
      {pendingRequests.length > 0 && (
        <SidebarSection title="Pending Requests" icon={Clock}>
            ...
            {pendingRequests.map((req) => (
              <div key={req.id} className="flex items-center gap-2.5 p-1.5 rounded-lg bg-secondary/50">
                <img
                  ...
                  alt={req.profile?.display_name || "User"}
                  className="w-9 h-9 rounded-full object-cover flex-shrink-0"
                />
                <div className="min-w-0">
                  <p className="text-[13px] font-medium text-foreground truncate">
                    {req.profile?.display_name || "User"}
                  </p>
      ...
      {suggestions.length > 0 && (
        <SidebarSection title="People You May Know" icon={UserPlus}>
          <div className="space-y-1">
            {suggestions.map((person) => (
              <div key={person.user_id} className="flex items-center gap-2.5 p-1.5 rounded-lg hover:bg-secondary transition-colors">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-muted-foreground line-clamp-1">{person.bio || "No bio"}</p>
                </div>
              </div>
            ))}
          </div>

...
const GlobalSearch = () => {
  ...
  const tabs: { key: Tab; label: string; icon: React.ElementType; count: number }[] = [
    ...
    { key: "people", label: "People", icon: User, count: results.people.length },
    { key: "posts", label: "Posts", icon: FileText, count: results.posts.length },
    { key: "conversations", label: "Chats", icon: MessageSquare, count: results.conversations.length },
  ];
        ...
        <div className="fixed sm:absolute inset-x-2 sm:inset-x-auto top-14 sm:top-0 sm:left-0 sm:w-[360px] bg-card rounded-lg shadow-xl border border-border z-[60] overflow-hidden">
          ...
          <div className="max-h-[400px] overflow-y-auto">
            ...
            {!loading && showPeople && results.people.length > 0 && (
              <div>
                {tab === "all" && <div className="px-3 pt-3 pb-1 text-xs font-semibold text-muted-foreground uppercase tracking-wide">People</div>}
            ...
            {!loading && showPosts && results.posts.length > 0 && (
              <div>
                {tab === "all" && <div className="px-3 pt-3 pb-1 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Posts</div>}
            ...
            {!loading && showConversations && results.conversations.length > 0 && (
              <div>
                {tab === "all" && <div className="px-3 pt-3 pb-1 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Chats</div>}
                {results.conversations.map((conv) => (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">{conv.is_group ? "Group chat" : "Direct message"}</p>
                    </div>

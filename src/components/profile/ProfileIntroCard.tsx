...
const ProfileIntroCard = ({ profile, profileUserId, isOwn, editingBio, setEditingBio }: ProfileIntroCardProps) => {
  ...
  const infoItems = [
    { icon: Briefcase, value: profile?.workplace, prefix: "Works at " },
    { icon: GraduationCap, value: profile?.education, prefix: "Studied at " },
    { icon: MapPin, value: profile?.location, prefix: "Lives in " },
    { icon: Globe, value: profile?.website, isLink: true },
    { icon: Heart, value: profile?.relationship_status },
    { icon: Calendar, value: `Joined ${profile?.created_at ? formatDistanceToNow(new Date(profile.created_at), { addSuffix: true }) : "recently"}`, alwaysShow: true },
  ];

  return (
    <div className="bg-card rounded-xl shadow-sm p-4 border border-border/50">
      <h2 className="text-base font-bold text-foreground mb-3">Intro</h2>

      {editingBio ? (
        <div className="space-y-2.5">
          {[
            { label: "Display Name", value: nameText, set: setNameText, max: 50 },
            { label: "Bio", value: bioText, set: setBioText, max: 500, multiline: true },
            { label: "Workplace", value: workplaceText, set: setWorkplaceText, max: 100 },
            { label: "Education", value: educationText, set: setEducationText, max: 100 },
            { label: "Location", value: locationText, set: setLocationText, max: 100 },
            { label: "Website", value: websiteText, set: setWebsiteText, max: 200 },
            { label: "Relationship Status", value: relationshipText, set: setRelationshipText, max: 50 },
          ].map((field) => (
            <div key={field.label}>
        ...
        <>
          ...
          <div className="space-y-2 text-sm mb-3">
            ...
            <div className="flex items-center gap-2.5">
              ...
              {online ? (
                <span className="text-green-500 font-medium text-[13px]">Active now</span>
              ) : profile?.last_seen_at ? (
                <span className="text-[13px] text-muted-foreground">Last seen {formatDistanceToNow(new Date(profile.last_seen_at), { addSuffix: true })}</span>
              ) : (
                <span className="text-[13px] text-muted-foreground">Last seen recently</span>
              )}
            </div>
          </div>

...
const Post = ({ id, postUserId, author, avatarUrl, createdAt, updatedAt, content, image, imageUrls, videoUrl, commentCount, sharedPostId, sharedGroupPostId, sharedPagePostId, memoryYearsAgo, groupId, groupName, groupAvatarUrl, pageId, pageName, pageSlug, pageAvatarUrl, privacy, backgroundStyle, location, feeling }: PostProps) => {
  ...
  const handleTogglePin = async () => {
    ...
    if (error) {
      ...
      toast.success(newVal ? "Post pinned to your profile" : "Post unpinned");
  ...
  const handleToggleArchive = async () => {
    ...
    if (error) {
      ...
      toast.success(newVal ? "Post moved to archive" : "Post restored from archive");
  ...
  const handleInterest = async (interested: boolean) => {
    ...
    try {
      ...
      toast.success(interested ? "We'll show you more posts like this" : "We'll show you fewer posts like this");
    } catch {
  ...
  const handleSnoozeUser = async () => {
    ...
    try {
      ...
      toast.success(`Snoozed ${author.split(" ")[0]} for 30 days`);
      queryClient.invalidateQueries({ queryKey: ["posts"] });
  ...
  const handleBlockUser = async () => {
    ...
    try {
      ...
      toast.success(`${author.split(" ")[0]} has been blocked`);
      queryClient.invalidateQueries({ queryKey: ["posts"] });
  ...
  const handleToggleCommenting = async () => {
    ...
    const { error } = await supabase
      ...
      .eq("id", id);
    if (error) {
      toast.error("Failed to update commenting setting");
    } else {
      toast.success(newVal ? "Commenting turned off" : "Commenting turned on");
  ...
  const handleTogglePostNotifications = async () => {
    ...
    try {
      if (isNotifSubscribed) {
        ...
        toast.success("You'll get notifications for this post");
      }
  ...
  const handleReact = async (reactionType: string) => {
    ...
    try {
      if (userLike && userLike.reaction_type === reactionType) {
        ...
        // Change reaction
        await supabase.from("post_likes").update({ reaction_type: reactionType }).eq("id", userLike.id);
      } else {
        // New reaction
        await supabase.from("post_likes").insert({ post_id: id, user_id: user.id, reaction_type: reactionType });
    ...
    <div className="bg-card rounded-lg shadow-sm">
      {memoryYearsAgo && (
        <div className="flex items-center gap-2 px-4 pt-3 pb-1">
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-primary/10 text-primary text-xs font-semibold">
            ...
            On This Day · {memoryYearsAgo} {memoryYearsAgo === 1 ? 'year' : 'years'} ago
          </div>
        </div>
      )}
      {/* Group badge */}
        ...
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap min-w-0">
              ...
              {location && (
                <span className="text-[13px] text-muted-foreground inline-flex items-center gap-0.5 flex-wrap min-w-0">
                  {feeling ? "at" : "is at"} <MapPin className="w-3 h-3 inline shrink-0" />
              ...
              {sharedPostId && (
                <span className="text-[13px] text-muted-foreground flex items-center gap-1 flex-wrap">
                  <Repeat2 className="w-3.5 h-3.5 shrink-0" /> shared a post
                </span>
              )}
              {sharedGroupPostId && !sharedPostId && !sharedPagePostId && (
                <span className="text-[13px] text-muted-foreground flex items-center gap-1 flex-wrap">
                  <Repeat2 className="w-3.5 h-3.5 shrink-0" /> shared a group post
                </span>
              )}
              {sharedPagePostId && !sharedPostId && !sharedGroupPostId && (
                <span className="text-[13px] text-muted-foreground flex items-center gap-1 flex-wrap">
                  <Repeat2 className="w-3.5 h-3.5 shrink-0" /> shared a page post
                </span>
              )}
            </div>
            <div className="flex items-center gap-1 text-[13px] text-muted-foreground flex-wrap min-w-0">
              ...
              {wasEdited && <span className="ml-1 text-muted-foreground">· edited</span>}
            </div>
          </div>
        </div>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
           ...
           <DropdownMenuContent
              ...
              <>
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium leading-tight">Interested</p>
                    <p className="text-xs text-muted-foreground leading-tight mt-0.5">Show more posts like this</p>
                  </div>
                </DropdownMenuItem>
                <DropdownMenuItem
                  className="cursor-pointer gap-3 px-3 py-2.5 rounded-lg transition-colors duration-150 focus:bg-accent/60"
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium leading-tight">Not interested</p>
                    <p className="text-xs text-muted-foreground leading-tight mt-0.5">Show fewer posts like this</p>
                  </div>
                </DropdownMenuItem>
              </>
            )}
            ...
            <DropdownMenuItem className="cursor-pointer gap-3 px-3 py-2.5 rounded-lg transition-colors duration-150 focus:bg-accent/60" onClick={handleToggleSave}>
              ...
              <div className="min-w-0">
                <p className="text-sm font-medium leading-tight">{isSaved ? "Unsave post" : "Save post"}</p>
                <p className="text-xs text-muted-foreground leading-tight mt-0.5">{isSaved ? "Remove from your saved items" : "Add to your saved items"}</p>
              </div>
            </DropdownMenuItem>
            ...
            >
              ...
              <div className="min-w-0">
                <p className="text-sm font-medium leading-tight">Copy link</p>
                <p className="text-xs text-muted-foreground leading-tight mt-0.5">Copy post URL to clipboard</p>
              </div>
            </DropdownMenuItem>
            ...
            >
              ...
              <div className="min-w-0">
                <p className="text-sm font-medium leading-tight">Embed post</p>
                <p className="text-xs text-muted-foreground leading-tight mt-0.5">Get embed code with preview</p>
              </div>
            </DropdownMenuItem>
            ...
            >
              ...
              <div className="min-w-0">
                <p className="text-sm font-medium leading-tight">{isNotifSubscribed ? "Turn off notifications" : "Turn on notifications"}</p>
                <p className="text-xs text-muted-foreground leading-tight mt-0.5">{isNotifSubscribed ? "Stop getting notified about this post" : "Get notified about activity"}</p>
              </div>
            </DropdownMenuItem>
            ...
            {canEdit && (
              <>
                ...
                <DropdownMenuItem className="cursor-pointer gap-3 px-3 py-2.5 rounded-lg transition-colors duration-150 focus:bg-accent/60" onClick={() => setShowEditModal(true)}>
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium leading-tight">Edit post</p>
                    <p className="text-xs text-muted-foreground leading-tight mt-0.5">Modify content or images</p>
                  </div>
                </DropdownMenuItem>
                ...
                <DropdownMenuItem className="cursor-pointer gap-3 px-3 py-2.5 rounded-lg transition-colors duration-150 focus:bg-accent/60" onClick={() => setShowBoostModal(true)}>
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium leading-tight">Boost post</p>
                    <p className="text-xs text-muted-foreground leading-tight mt-0.5">Reach more people with this post</p>
                  </div>
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem className="cursor-pointer gap-3 px-3 py-2.5 rounded-lg transition-colors duration-150 focus:bg-accent/60" onClick={handleTogglePin} disabled={pinToggling}>
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium leading-tight">{isPinned ? "Unpin post" : "Pin post"}</p>
                    <p className="text-xs text-muted-foreground leading-tight mt-0.5">{isPinned ? "Remove from top of your profile" : "Pin to top of your profile"}</p>
                  </div>
                </DropdownMenuItem>
                ...
                <DropdownMenuItem className="cursor-pointer gap-3 px-3 py-2.5 rounded-lg transition-colors duration-150 focus:bg-accent/60" onClick={() => setShowEditAudienceModal(true)}>
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium leading-tight">Edit audience</p>
                    <p className="text-xs text-muted-foreground leading-tight mt-0.5">Change who can see this post</p>
                  </div>
                </DropdownMenuItem>
                ...
                <DropdownMenuItem className="cursor-pointer gap-3 px-3 py-2.5 rounded-lg transition-colors duration-150 focus:bg-accent/60" onClick={handleToggleCommenting} disabled={commentToggling}>
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium leading-tight">{commentsDisabled ? "Turn on commenting" : "Turn off commenting"}</p>
                    <p className="text-xs text-muted-foreground leading-tight mt-0.5">{commentsDisabled ? "Allow others to comment again" : "Prevent others from commenting"}</p>
                  </div>
                </DropdownMenuItem>
                ...
                <DropdownMenuItem className="cursor-pointer gap-3 px-3 py-2.5 rounded-lg transition-colors duration-150 focus:bg-accent/60" onClick={() => setShowEditDateModal(true)}>
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium leading-tight">Edit date</p>
                    <p className="text-xs text-muted-foreground leading-tight mt-0.5">Change the date on this post</p>
                  </div>
                </DropdownMenuItem>
                ...
                <DropdownMenuItem className="cursor-pointer gap-3 px-3 py-2.5 rounded-lg transition-colors duration-150 focus:bg-accent/60" onClick={handleToggleArchive}>
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium leading-tight">{isArchived ? "Restore from archive" : "Move to archive"}</p>
                    <p className="text-xs text-muted-foreground leading-tight mt-0.5">{isArchived ? "Show this post on your timeline again" : "Archive this post from your timeline"}</p>
                  </div>
                </DropdownMenuItem>
                ...
                <DropdownMenuItem className="cursor-pointer gap-3 px-3 py-2.5 rounded-lg transition-colors duration-150 focus:bg-accent/60" onClick={() => setShowEditHistoryModal(true)}>
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium leading-tight">View edit history</p>
                    <p className="text-xs text-muted-foreground leading-tight mt-0.5">See previous versions of this post</p>
                  </div>
                </DropdownMenuItem>
              </>
            )}
            ...
            {!isOwner && (
              <>
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium leading-tight">Hide post</p>
                    <p className="text-xs text-muted-foreground leading-tight mt-0.5">Remove from your feed</p>
                  </div>
                </DropdownMenuItem>
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium leading-tight">Snooze {author.split(" ")[0]} for 30 days</p>
                    <p className="text-xs text-muted-foreground leading-tight mt-0.5">Temporarily stop seeing posts</p>
                  </div>
                </DropdownMenuItem>
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium leading-tight">{isFollowing ? `Unfollow ${author.split(" ")[0]}` : `Follow ${author.split(" ")[0]}`}</p>
                    <p className="text-xs text-muted-foreground leading-tight mt-0.5">{isFollowing ? "Stop seeing posts but stay friends" : "See their posts in your feed"}</p>
                  </div>
                </DropdownMenuItem>
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium leading-tight">{translating ? "Translating…" : translatedContent ? "Show original" : "Translate post"}</p>
                    <p className="text-xs text-muted-foreground leading-tight mt-0.5">{translatedContent ? "View the original content" : "See this post in your language"}</p>
                  </div>
                </DropdownMenuItem>
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium leading-tight">Download media</p>
                    <p className="text-xs text-muted-foreground leading-tight mt-0.5">Save images or videos to device</p>
                  </div>
                </DropdownMenuItem>
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium leading-tight">Find support</p>
                    <p className="text-xs text-muted-foreground leading-tight mt-0.5">Get help or contact resources</p>
                  </div>
                </DropdownMenuItem>
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium leading-tight">Report post</p>
                    <p className="text-xs text-muted-foreground leading-tight mt-0.5">Report content that's inappropriate</p>
                  </div>
                </DropdownMenuItem>
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium leading-tight">Block {author.split(" ")[0]}</p>
                    <p className="text-xs opacity-70 leading-tight mt-0.5">Stop all interactions with this user</p>
                  </div>
                </DropdownMenuItem>
              </>
            )}
            ...
            {isOwner && (
              <>
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium leading-tight">Delete post</p>
                    <p className="text-xs opacity-70 leading-tight mt-0.5">Permanently remove this post</p>
                  </div>
                </DropdownMenuItem>
              </>
            )}
        ...
        <div className="mx-4 mb-2 rounded-xl bg-secondary/40 overflow-hidden cursor-pointer hover:bg-secondary/60 transition-colors" onClick={() => window.open(`/?post=${sharedPost.id}`, "_blank")}>
          <div className="p-3 flex items-center gap-2.5">
            <img
              ...
              alt={sharedPost.profiles?.display_name || "User"}
              className="w-9 h-9 rounded-full object-cover ring-2 ring-background"
            />
            <div className="flex-1 min-w-0">
              <p className="text-[13px] font-semibold text-foreground truncate">
                {sharedPost.profiles?.display_name || "Unknown User"}
              </p>
        ...
        <div className="mx-4 mb-2 rounded-xl bg-secondary/40 overflow-hidden cursor-pointer hover:bg-secondary/60 transition-colors" onClick={() => window.open(`/groups/${sharedGroupPost.group_id}`, "_blank")}>
          ...
          <div className="px-3 pt-2.5 pb-1.5 flex items-center justify-between">
            <GroupHoverCard
              ...
              groupName={sharedGroupPost.group?.name || "Group"}
            ...
            >
              <div className="flex items-center gap-2 cursor-pointer w-fit" onClick={(e) => e.stopPropagation()}>
                ...
                <span className="text-xs font-semibold text-primary hover:underline">{sharedGroupPost.group?.name || "Group"}</span>
              </div>
            </GroupHoverCard>
          ...
          <div className="px-3 pb-1.5 flex items-center gap-2">
            <img
              ...
              alt={sharedGroupPost.profiles?.display_name || "User"}
              className="w-7 h-7 rounded-full object-cover"
            />
            <div className="flex-1 min-w-0">
              <p className="text-[13px] font-semibold text-foreground truncate">
                {sharedGroupPost.profiles?.display_name || "Unknown User"}
              </p>
              <p className="text-[11px] text-muted-foreground">
                {formatDistanceToNow(new Date(sharedGroupPost.created_at), { addSuffix: true })}
              </p>
            </div>
          </div>
        ...
        <div className="mx-4 mb-2 rounded-xl bg-secondary/40 overflow-hidden cursor-pointer hover:bg-secondary/60 transition-colors" onClick={() => window.open(`/pages/${sharedPagePost.page?.slug}`, "_blank")}>
          <div className="px-3 pt-2.5 pb-1.5 flex items-center justify-between">
            <a href={`/pages/${sharedPagePost.page?.slug}`} className="flex items-center gap-2 w-fit" onClick={(e) => e.stopPropagation()}>
              ...
              <span className="text-xs font-semibold text-primary hover:underline">{sharedPagePost.page?.name || "Page"}</span>
            </a>
            <ExternalLink className="w-3.5 h-3.5 text-muted-foreground" />
          </div>
          <div className="px-3 pb-1 text-[11px] text-muted-foreground">
            {formatDistanceToNow(new Date(sharedPagePost.created_at), { addSuffix: true })}
          </div>
      ...
      <div className="mx-4 border-t border-border py-1 flex items-center justify-around">
          ...
          >
            {userLike ? (
              <>
                ...
                <span className="font-semibold text-[15px]">{REACTIONS.find((r) => r.type === userLike.reaction_type)?.label || "Like"}</span>
              </>
            ) : (
              <>
        ...
        <button
          ...
          title={commentsDisabled ? "Commenting is turned off for this post" : undefined}
        >
      ...
      {showComments && !commentsDisabled && (
        <ThreadedComments postId={id} postUserId={postUserId} />
      )}
      {commentsDisabled && (
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-3">
                ...
                <ul className="text-sm space-y-1 text-muted-foreground list-disc pl-4">
                  <li>The post and all its content</li>

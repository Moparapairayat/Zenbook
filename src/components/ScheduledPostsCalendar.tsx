...
const ScheduledPostsCalendar = ({ scheduledPosts, pageId, onPublishNow, onClickSlot }: ScheduledPostsCalendarProps) => {
  ...
  const handleDrop = useCallback(async (e: DragEvent, targetDay: Date) => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["page-posts"] });
      toast.success(`Post moved to ${format(newDate, "MMM d, yyyy")}`);
    } catch {
      ...
      <div className="flex items-center gap-3 flex-wrap justify-between">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="bg-secondary rounded-lg px-3 py-1.5 text-sm text-muted-foreground flex items-center gap-1.5">
            ...
            {scheduledPosts.length} total scheduled
          </div>
          <div className="bg-secondary rounded-lg px-3 py-1.5 text-sm text-muted-foreground flex items-center gap-1.5">
      ...
      {bulkMode && (
        <div className="bg-card rounded-xl border border-border p-3 flex items-center gap-3 flex-wrap animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="flex items-center gap-2">
            ...
            >
              {selectedPostIds.size === scheduledPosts.length ? "Deselect all" : "Select all"}
            </button>
            <span className="text-xs text-muted-foreground">
              {selectedPostIds.size} of {scheduledPosts.length} selected
            </span>
          </div>
          ...
          >
            ...
            Publish {selectedPostIds.size > 0 ? `(${selectedPostIds.size})` : ""}
          </button>
          <button
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This action cannot be undone. The selected scheduled post{selectedPostIds.size > 1 ? "s" : ""} will be permanently deleted.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
      ...
      {scheduledPosts.length > 0 && (
        <div className="bg-card rounded-xl border border-border overflow-hidden">
          <div className="p-4 border-b border-border">
            <h4 className="text-sm font-bold text-foreground">All Scheduled Posts</h4>
          </div>
          <div className="divide-y divide-border">
            {[...scheduledPosts]
              .sort((a, b) => new Date(a.scheduled_at).getTime() - new Date(b.scheduled_at).getTime())
                    ...
                    {showDateHeader && (
                      <div className="px-4 py-2 bg-secondary/50">
                        <span className={cn("text-xs font-bold", isExpired ? "text-muted-foreground" : "text-foreground")}>
                          {format(postDate, "EEEE, MMMM d, yyyy")}
                        </span>
                      </div>
                    )}
                    <div

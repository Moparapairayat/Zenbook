  ...
  const handleShareToTimeline = async () => {
    ...
    try {
      const insertData: any = {
        user_id: user.id,
        content: shareText.trim() || `Shared a post by ${authorLabel}`,
      };
    ...
    try {
      for (const friendId of selectedFriends) {
        ...
        const messageContent = shareText.trim()
          ? `${shareText.trim()}\n\n📎 ${authorLabel}: ${postUrl}`
          : `📎 Shared a post by ${authorLabel}: ${postUrl}`;
      ...
      toast.success(
        selectedFriends.length === 1 ? "Sent to friend!" : `Sent to ${selectedFriends.length} friends!`
      );
      onClose();
    } catch (error: any) {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 px-4 animate-in fade-in duration-150" onClick={onClose}>
      ...
      >
        ...
        <div className="flex px-5 pt-2 gap-1">
          {[
            { key: "timeline" as const, icon: Repeat2, label: "Timeline" },
            { key: "friend" as const, icon: Users, label: "Friends" },
          ...
          >
            {loading ? (
              ...
              <>
                ...
                Send{selectedFriends.length > 1 ? ` (${selectedFriends.length})` : ""}
              </>
            )}
          </button>
        </div>

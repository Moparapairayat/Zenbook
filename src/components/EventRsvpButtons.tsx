...
const RSVP_OPTIONS: { status: RsvpStatus; label: string; icon: typeof Check; activeClass: string }[] = [
  { status: "going", label: "Going", icon: Check, activeClass: "bg-green-500/20 text-green-400 border-green-500/50" },
  { status: "interested", label: "Interested", icon: Star, activeClass: "bg-yellow-500/20 text-yellow-400 border-yellow-500/50" },
  { status: "not_going", label: "Can't Go", icon: X, activeClass: "bg-red-500/20 text-red-400 border-red-500/50" },
];
    ...
    try {
      ...
      if (existing) {
        ...
        if (remindAt <= new Date()) {
          toast.error("This reminder time has already passed");
          return;
        }
      ...
      toast.success(existing ? "Reminder removed" : "Reminder set!");
    } catch {
        ...
        {eventInFuture && (
          <Popover open={showReminders} onOpenChange={setShowReminders}>
            ...
            <PopoverContent className="w-52 p-2" align="start">
              <p className="text-xs font-semibold text-foreground px-1 mb-2">Set reminders</p>

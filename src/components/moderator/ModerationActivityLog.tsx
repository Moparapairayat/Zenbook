...
const actionLabels: Record<string, string> = {
  ...
  ban_revoked: "Ban/Warning Revoked",
...
const ModerationActivityLog = () => {
  ...
  const formatDetails = (details: Record<string, any>): string => {
    ...
    if (details.review_note) parts.push(`Note: ${details.review_note}`);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <h2 className="text-xl font-bold text-foreground">Moderation Activity Log</h2>
          ...
          {loading ? (
            ...
            <Table>
              ...
              <TableBody>
                {logs.map((entry) => (
                  <TableRow key={entry.id}>
                    <TableCell className="text-sm font-medium">
                      {entry.moderator_profile?.display_name || "Unknown"}
                    </TableCell>
                    <TableCell>
                ...
                {logs.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">No moderation activity found.</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>

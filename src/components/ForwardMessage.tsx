  ...
  const { data: conversations } = useQuery({
    ...
    queryFn: async () => {
      ...
      groupConvs.forEach((c: any) => {
        options.push({ id: c.id, label: c.group_name || "Group Chat", avatar: c.group_avatar_url || null, is_group: true });
      });
      return options;
    },
    enabled: open && !!user,
          ...
          <div className="max-h-64 overflow-y-auto rounded-lg border border-border divide-y divide-border">
            ...
            {filtered?.map((conv) => (
              ...
              >
                ...
                {conv.is_group && (
                  <span className="text-[11px] text-muted-foreground bg-muted px-1.5 py-0.5 rounded">Group</span>
                )}

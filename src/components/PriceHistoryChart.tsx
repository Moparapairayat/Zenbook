...
const PriceHistoryChart = ({ listingId, currentPrice }: PriceHistoryChartProps) => {
  ...
  const chartData = history.map((h) => ({
    date: format(new Date(h.changed_at), "MMM d"),
    fullDate: format(new Date(h.changed_at), "MMM d, yyyy 'at' h:mm a"),
  ...
  return (
    <div className="rounded-lg bg-card shadow-sm p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        {priceDiff !== 0 && (
          <div className={`flex items-center gap-1 text-xs font-medium ${isDown ? "text-green-500" : "text-red-500"}`}>
            ...
            {isDown ? "-" : "+"}${Math.abs(priceDiff).toLocaleString()}
          </div>
        )}
      </div>
      <div className="h-40">
      ...
      <p className="text-[11px] text-muted-foreground mt-2">
        {chartData.length} price point{chartData.length !== 1 ? "s" : ""} recorded
      </p>
    </div>
  );
};

...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const [userId, events] of userEvents) {
      ...
      const extra = eventCount > 3 ? ` and ${eventCount - 3} more` : "";
      const statusParts: string[] = [];
      if (goingCount > 0) statusParts.push(`${goingCount} going`);
      if (interestedCount > 0) statusParts.push(`${interestedCount} interested`);

      const message = `📅 Your week ahead: ${eventCount} event${eventCount !== 1 ? "s" : ""} (${statusParts.join(", ")}). ${eventTitles}${extra}`;

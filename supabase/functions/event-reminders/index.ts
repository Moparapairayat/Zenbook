    ...
    if (dueReminders && dueReminders.length > 0) {
      ...
      for (const reminder of dueReminders) {
        ...
        const label = reminder.remind_minutes_before >= 1440
          ? `${Math.round(reminder.remind_minutes_before / 1440)} day`
          : reminder.remind_minutes_before >= 60
          ? `${Math.round(reminder.remind_minutes_before / 60)} hour`
        ...
        notifications.push({
          ...
          type: "event",
          message: `Reminder (${label}): "${event.title}" at ${timeStr}`,
        ...
        if (rsvps && rsvps.length > 0) {
          ...
          const notifications = rsvps
            ...
            .map((r: any) => ({
              ...
              actor_id: event.created_by,
              type: "event",
              message: `Reminder: "${event.title}" starts at ${timeStr}`,
              reference_id: event.id,
            }));

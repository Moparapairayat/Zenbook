...
Deno.serve(async (req) => {
  ...
  if (error || !post) {
    return new Response(renderErrorHtml("Post not found", theme), {
      status: 404,
  ...
  if (post.privacy !== "public") {
    return new Response(renderErrorHtml("This post is private", theme), {
      status: 403,
      headers: { ...corsHeaders, "Content-Type": "text/html; charset=utf-8" },
    });
  }
...
function renderPostHtml(post: { id: string; content: string; image_url: string | null; image_urls: string[] | null; created_at: string }, profile: { display_name: string | null; avatar_url: string | null } | null, theme: string, showMedia: boolean): string {
  ...
  const imageHtml = showMedia && imageUrl ? `<div style="margin-top:12px"><img src="${escapeHtml(imageUrl)}" alt="Post image" style="width:100%;max-height:400px;object-fit:cover;border-radius:8px"></div>` : "";
  ...
  const imageSection = showMedia && imageUrl ? `<div style="margin-top:12px;position:relative"><img src="${escapeHtml(imageUrl)}" alt="Post image" style="width:100%;max-height:400px;object-fit:cover;border-radius:8px">${multiImageBadge}</div>` : "";

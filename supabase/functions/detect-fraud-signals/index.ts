    ...
    for (const listing of listings) {
      ...
      if (avg > 0 && price < avg * 0.3 && price > 0) {
        signals.push({
          ...
          severity: "high",
          description: `Price ($${price}) is ${Math.round((1 - price / avg) * 100)}% below category average ($${Math.round(avg)})`,
      ...
      if (accountAgeDays < 7 && userCount > 10) {
        signals.push({
          ...
          severity: "high",
          description: `New account (${Math.round(accountAgeDays)} days old) with ${userCount} active listings`,
      ...
      if (accountAgeDays < 14 && userCount > 5 && !(accountAgeDays < 7 && userCount > 10)) {
        signals.push({
          ...
          severity: "medium",
          description: `Recent account (${Math.round(accountAgeDays)} days old) with ${userCount} active listings`,
          metadata: { account_age_days: Math.round(accountAgeDays), listing_count: userCount },
        });
      }
      ...
      if (price > 0 && price < 1 && avg > 10) {
        signals.push({
          ...
          severity: "high",
          description: `Price ($${price}) is suspiciously low for ${listing.category} category`,
      ...
      if (price > 500 && (!listing.description || listing.description.trim().length < 10)) {
        signals.push({
          ...
          severity: "low",
          description: `High-value listing ($${price}) with minimal or no description`,
          metadata: { price },
        });
      }
    }
